...
public sealed class LinqExercises
{
    ...
    {
        return UniversityData.Students.Where(s => s.City == "Warsaw").Select(s => $"{s.IndexNumber} {s.FirstName} {s.LastName} {s.City}");
    }
    ...
    {
        return UniversityData.Students.OrderBy(s => s.LastName).ThenBy(s => s.FirstName).Select(s => $"{s.IndexNumber} {s.FirstName} {s.LastName}");
    }
    ...
    {
        yield return UniversityData.Enrollments.Where(e => e.IsActive).Count().ToString();
    }
    ...
    {
        return UniversityData.Enrollments.OrderByDescending(e => e.EnrollmentDate).Take(3).Select(e => $"{e.EnrollmentDate} {e.StudentId} {e.CourseId}");
    }
    ...
    {
        return UniversityData.Enrollments.SelectMany(e => UniversityData.Students.Where(s => s.Id == e.StudentId),(e,s) => new { e,s })
                                         .SelectMany(es => UniversityData.Courses.Where(c => c.Id == es.e.CourseId),(es,c) => $"{es.s.FirstName} {es.s.LastName} {c.Title}");
    }
